...
export default function Cart() {
  const { groupedByVendor, total, removeItem, updateQty, items } = useCart();
  ...
  const handleCheckout = async () => {
    try {
      const payload = {
        ...
        shippingAddress: {} // wire up a real address form later
      };
      const res = await checkout(payload);
      navigate('/checkout', { state: { orders: res.data.orders } });
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: 480, margin: '2rem auto', padding: '0 1rem' }}>
      ...
      {groupedByVendor.map((group) => (
        <div key={group.vendorId} style={{ border: '1px solid #e5e5e5', borderRadius: 12, padding: '0.75rem 1rem', marginBottom: 12 }}>
          <p style={{ fontSize: 13, color: '#666', marginBottom: 8 }}>Sold by {group.vendorName}</p>
          {group.items.map((item) => (
            <div key={item.productId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderTop: '1px solid #eee' }}>
              <div>
                <p style={{ fontSize: 14, margin: 0 }}>{item.title}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ borderTop: '1px solid #ccc', paddingTop: 12, display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
        <p style={{ fontSize: 14, color: '#666' }}>Total ({groupedByVendor.length} order{groupedByVendor.length > 1 ? 's' : ''})</p>
        <p style={{ fontSize: 20, fontWeight: 500 }}>${(total / 100).toFixed(2)}</p>
      </div>
