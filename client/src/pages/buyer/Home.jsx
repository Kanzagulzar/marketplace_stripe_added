...
export default function Home() {
  ...
  return (
    <div className="marketplace-page" style={{ maxWidth: 720, margin: '2rem auto', padding: '0 1rem' }}>
      <div className="marketplace-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 500, margin: 0 }}>Welcome, {user.name}</h1>
          <p style={{ fontSize: 13, color: '#666', margin: 0 }}>Role: {user.role}</p>
        </div>
        <div className="nav-actions" style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <Link to="/" style={{ fontSize: 14, color: '#185fa5' }}>Home</Link>
          <Link to="/cart" style={{ fontSize: 14, color: '#185fa5' }}>
            Cart {items.length > 0 && `(${items.reduce((sum, i) => sum + i.qty, 0)})`}
          </Link>
          {user.role === 'buyer' && (
            <Link to="/orders" style={{ fontSize: 14, color: '#185fa5' }}>
              Orders
            </Link>
          )}
          <button onClick={logout} style={{ padding: '8px 16px', fontSize: 14 }}>Log out</button>
      ...
      <div className="section-heading">
        <div>
          ...
          <h2 style={{ fontSize: 16, fontWeight: 500, marginTop: '2rem', marginBottom: 12 }}>Popular products</h2>
        </div>
      </div>
        ...
        {products.map((p) => (
          ...
          >
            ...
            <div style={{ padding: 12 }}>
              <p style={{ fontSize: 14, fontWeight: 500, margin: '0 0 4px' }}>{p.title}</p>
              <p style={{ fontSize: 13, color: '#666', margin: 0 }}>${(p.price / 100).toFixed(2)}</p>
              <p style={{ fontSize: 12, color: '#999', margin: '4px 0 0' }}>Sold by {p.vendorId?.name || p.vendorId?.storeName}</p>
            </div>
          </Link>
        ))}
      </div>
